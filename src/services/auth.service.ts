...
export class AuthService {
  ...
  public getCookiesWithJwtToken(userId: number) {
    ...
    return `Authentication=${token}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=${process.env.JWT_ACCESS_TOKEN_EXPIRATION_TIME}`;
  }
  ...
  public getCookieWithJwtRefreshToken(userId: number) {
    ...
    const cookie = `Refresh=${token}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=${process.env.JWT_REFRESH_TOKEN_EXPIRATION_TIME}`;
    return {
      cookie,
      token,
    };
