...
export class AuthService {
  ...
  public getCookiesWithJwtToken(userId: number) {
    ...
    return `Authentication=${token}; HttpOnly; Secure=false; SameSite=None Path=/; Max-Age=${process.env.JWT_ACCESS_TOKEN_EXPIRATION_TIME}`;
  }
  ...
  public getCookieWithJwtRefreshToken(userId: number) {
    ...
    const cookie = `Refresh=${token}; HttpOnly; Secure=true; SameSite=none Path=/; Max-Age=${process.env.JWT_REFRESH_TOKEN_EXPIRATION_TIME}`;
    return {
      cookie,
      token,
    };
